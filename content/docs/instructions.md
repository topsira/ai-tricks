...
ทุก Note จะถูกแยกตามโฟลเดอร์ใน `content/docs/` เพื่อให้ Sidebar สร้างโครงสร้างหมวดหมู่อัตโนมัติ โดยตั้งชื่อไฟล์เป็น `topic-name.md` (ตัวพิมพ์เล็ก คั่นด้วย hyphen) ไม่ต้องใส่ prefix หมวดนำหน้าชื่อไฟล์แล้ว:

| โฟลเดอร์       | หมวด                          | ตัวอย่าง                              |
|-------------|-------------------------------|---------------------------------------|
| `prompts/`  | Prompt Engineering            | `prompts/role-context-constraint.md`  |
| `tools/`    | AI Coding Tools               | `tools/claude-code-setup.md`          |
| `workflows/`| AI Co-working Workflow        | `workflows/ai-code-review.md`         |
| `skills/`   | Skills, Agents & Instructions | `skills/output-format-lock.md`        |
| `install/`  | Installation & Learning Notes | `install/gemini-cli.md`               |

Sidebar ของเว็บจะสร้างจากโครงสร้างโฟลเดอร์ย่อยเหล่านี้โดยอัตโนมัติ — เพิ่มไฟล์ลงในโฟลเดอร์ที่ถูกต้องแล้วขึ้นเมนูเอง

## Documentation Standard

ทุก Note ใช้โครงสร้าง 7 ส่วนนี้ (พร้อม frontmatter `title`):
...
และหากมีจุดที่ต้องระวังเป็นพิเศษ ให้ใช้ **Callout Component** เสมอ:
```markdown
{{< callout warning >}}
**คำเตือน / Safety Check:** ระบุสิ่งที่ต้องตรวจสอบก่อนนำโค้ดไปรันจริง
{{< /callout >}}
```
